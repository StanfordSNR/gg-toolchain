...
if [ ! -z ${DO_CONFIGURE+x} ]
then
  ../../binutils-gdb/configure --prefix=/usr --disable-gdb --disable-readline --disable-sim --enable-deterministic-archives --build=x86_64-linux-musl --host=x86_64-linux-musl --target=x86_64-linux-gnu --enable-gold=yes --enable-ld=no --program-prefix="gg-" CC="gnu-to-gg-gcc -Wl,-I${SRCDIR}/deps/lib/libc.so" CXX="gnu-to-gg-g++ -Wl,-I${SRCDIR}/deps/lib/libc.so"
fi
make -j${NCPU}
make DESTDIR=${SRCDIR}/inst install
popd
